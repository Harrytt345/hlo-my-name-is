...
def cleanup_temp_files():
    """Remove temporary files to manage disk space"""
    print("Starting cleanup...")
    try:
        # Clean temp directories
        temp_dirs = ['/tmp', './downloads', './temp']
        deleted_count = 0
        ...
        download_patterns = ['*.mp4', '*.mkv', '*.avi', '*.mp3', '*.pdf', '*.zip']
        for pattern in download_patterns:
            for file_path in Path('.').glob(pattern):
                try:
                    if time.time() - file_path.stat().st_ctime > 3600:  # 1 hour old
                        file_path.unlink()
                        deleted_count += 1
                except Exception as e:
                    print(f"Error deleting {file_path}: {e}")
...
def check_disk_space():
    ...
    try:
        ...
        # If less than 500MB free, trigger cleanup
        if free_gb < 0.5:
...
def clean_session_files():
    ...
    for file in session_files:
        try:
            ...
            if time.time() - os.path.getctime(file) > 86400:
                os.remove(file)
...
# Webhook and updates clearing function
async def clear_webhook_and_updates():
    ...
    try:
        # Clear pending updates by getting updates with offset -1
        await bot.get_updates(offset=-1)
        print("✅ Pending updates cleared")
    except Exception as e:
        print(f"⚠️ Error clearing updates: {e}")

# Cleanup before download function
async def cleanup_before_download():
    """Clean up temporary files and check disk space before starting a download"""
    print("🧹 Running pre-download cleanup...")
    cleanup_temp_files()
    check_disk_space()
    print("✅ Pre-download cleanup completed")
...
async def status_check(request):
    ...
    status_info = {
        ...
        "disk_space": f"{psutil.disk_usage('/').free / (1024**3):.2f}GB free"
    }
...
def validate_environment():
    ...
    required_vars = ['API_ID', 'API_HASH', 'BOT_TOKEN', 'OWNER']
    missing_vars = []
...
async def test_command(client, message):
    print(f"📨 TEST: Received /test from user {message.from_user.id}")
    await message.reply_text("✅ Bot is working perfectly!")

# Clear webhook command
@bot.on_message(filters.command("clearwebhook") & filters.private)
async def clear_webhook_command(client, message):
    """Command to manually clear webhook and updates"""
    if message.from_user.id != OWNER:
        return await message.reply("❌ Only the owner can use this command.")
    
    await message.reply("🔄 Clearing webhook and pending updates...")
    await clear_webhook_and_updates()
    await message.reply("✅ Webhook and updates cleared successfully!")

@bot.on_message(filters.command("start"))
async def start(bot, m: Message):
    print(f"🔍 DEBUG: Received /start from user {m.from_user.id}")
    user_id = m.chat.id
...
async def main():
    ...
    try:
        ...
        print("Clearing webhook and updates...")
        await clear_webhook_and_updates()
        ...
        print("Available at: https://hlo-my-name-is-5.onrender.com")
        print("Status endpoint: https://hlo-my-name-is-5.onrender.com/status")
